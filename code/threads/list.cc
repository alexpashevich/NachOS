...
#ifdef CHANGED
//----------------------------------------------------------------------
// List::findElemAndClean
//      Find the element with the help of comp function and delete all
...
//      False otherwise.
//----------------------------------------------------------------------
bool List::findElemAndClean(bool (*comp) (int*, int*), int* elem, time_t (*getTimestamp) (int*), time_t maxlag) {
    time_t now;
    time(&now);
    ListElement *prev = NULL;
    ListElement *prevprev = NULL;
    bool res = false;
    ...
    for (ListElement *ptr = first; ptr != NULL; ptr = ptr->next) {
        ...
        if ((*comp) ((int*) ptr->item, elem)) {
            res = res || true;
        }
        int val;
        if ((val = now - (*getTimestamp)((int*) ptr->item)) >= maxlag) {
            printf("deleted, val = %d\n", val);
            deletePrev = true;
        } else {
            printf("not deleted, val = %d\n", val);
            deletePrev = false;
        }
        if (prev) {
            prevprev = prev;
...
//----------------------------------------------------------------------
// List::findElemAndDelete
//      Find the element with the help of comp function and delete it.
// 
// Returns:
//      True if the element was found.
//      False otherwise.
//----------------------------------------------------------------------
bool List::findElemAndDelete (bool (*comp) (int*, int*), int* elem) {
    ListElement *prev = NULL;
    for (ListElement *ptr = first; ptr != NULL; ptr = ptr->next) {
        if ((*comp) ((int*) ptr->item, elem)) {
            if (prev == NULL)
